...
### Backfill 運用手順（T018 詳細）

1. 事前バックアップを取得する。
  - DB 全体バックアップと `Categories` テーブルのエクスポートを取得する。
2. 正規化プレビューを実行する。
  - `Name` から `NormalizedName` 候補を生成し、重複候補一覧を CSV で出力する。
3. 衝突抽出レポートを作成する。
  - `NormalizedName` が重複するレコードを抽出し、管理者レビュー対象として記録する。
4. 管理者レビューを実施する。
  - 同義カテゴリの統合方針（残すID、名称）を確定し、統合作業計画を承認する。
5. マイグレーションを適用する。
  - `NormalizedName` の backfill 実行後、UNIQUE 制約を追加する。
6. 失敗時の再実行手順を実施する。
  - バックアップから復元し、衝突解消後に再適用する。
7. 完了検証を行う。
  - `NormalizedName` null 件数 = 0、重複件数 = 0、カテゴリ参照整合性エラー = 0 を確認する。
