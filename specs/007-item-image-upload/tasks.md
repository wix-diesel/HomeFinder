...
- [ ] T1-001 [P] セットアップ: バックエンド SixLabors.ImageSharp NuGet パッケージをインストール | src/HomeFinder.Application/HomeFinder.Application.csproj
- [ ] T1-002 [P] セットアップ: バックエンド DotNext NuGet パッケージ（Result<T> サポート）をインストール | src/HomeFinder.Application/HomeFinder.Application.csproj
...
- [ ] T1-006 [P] セットアップ: Docker Azurite ローカル Blob Storage コンテナ起動確認（docker-compose.yml に追加） | docker-compose.yml
...
- [ ] T2-009 [P] 基礎: バックエンド Azure Blob Storage クライアント（BlobContainerClient）の DI 登録 | src/HomeFinder.Api/Program.cs
...
- [ ] T3-006 [US1] バックエンド: ImageService 実装 - 画像解像度取得（SixLabors.ImageSharp） | src/HomeFinder.Application/Services/ImageService.cs
- [ ] T3-007 [US1] バックエンド: ImageService 実装 - 画像を 1000x1000 にリサイズ（SixLabors.ImageSharp） | src/HomeFinder.Application/Services/ImageService.cs
- [ ] T3-008 [US1] バックエンド: ImageService 実装 - Azure Blob Upload 処理（BlobContainerClient.UploadBlobAsync） | src/HomeFinder.Application/Services/ImageService.cs
...
- [ ] T3-016 [US1] バックエンド: ImagesController - エラーレスポンスマッピング（Result.Error → HTTP Status） | src/HomeFinder.Api/Controllers/ImagesController.cs
...
- [ ] T3-025 [US1] フロントエンド: ItemEdit.vue に ImageUploader コンポーネント統合 | src/HomeFinder.UI/src/pages/ItemEdit.vue
...
- [ ] T4-009 [US2] バックエンド: ImagesController - バイナリ レスポンス返却（Content-Type, Cache-Control） | src/HomeFinder.Api/Controllers/ImagesController.cs
