...
// apiClient をモックして msalService の初期化エラーを回避する
...
describe('storage services', () => {
  ...
  it('listRooms が rooms 配列を返す', async () => {
    vi.mocked(apiClient.apiFetch).mockResolvedValue(new Response(JSON.stringify({ rooms: [{ id: 'r1' }] }), { status: 200 }));
  ...
  it('create/update/delete room を呼び出せる', async () => {
    vi.mocked(apiClient.apiFetch).mockImplementation(async () => new Response(JSON.stringify({ id: 'r1' }), { status: 200 }));
